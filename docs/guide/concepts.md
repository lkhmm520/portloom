...
同一 Server 进程包含两个监听器：管理监听器（默认 `127.0.0.1:8080`）提供控制台和 API；Gateway（默认 `127.0.0.1:8081`）按 HTTP `Host` 查找路由并代理到 SSH 回环端口。
...
HTTP 路由包含域名、本地目标、启用状态和自动分配的 VPS 回环端口。域名唯一且会规范化。TCP 路由在当前版本主要作为控制平面元数据。
