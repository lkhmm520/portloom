...
为避免 `latest` 等可变标签在拉取后破坏回滚，镜像引用未变化的幂等重跑不会再次 pull；升级必须传入新的固定版本。镜像标签变化时，安装器会先完整生成候选文件，再创建 `native-upgrade-backup-0.3.1/`（目录后缀是目标版本），其中保存升级前的 `.env` 与 `compose.yml`。新版本只有通过实际 HTTPS `/healthz` 验证才算成功；失败时安装器会恢复旧 Compose、旧配置并验证旧 HTTPS。若自动恢复无法验证，必须按报错人工检查。成功后保留该备份目录，确认稳定后可归档或删除。
...
固定旧镜像标签并使用已备份的 Compose 重建。数据库变更前必须先备份；入口回滚还必须恢复原公网入口和上游映射，不能只启动旧隧道容器。
