...
原生入口按 Host 把 `TM_PUBLIC_HOST` 发送到控制处理器，把其他域名发送到 Gateway。公网管理请求具有 30 秒读写期限和 1 MiB 请求体上限，避免慢速请求占用连接；这些限制只作用于管理 Host，Gateway 业务路由仍可进行长时间流式传输。Gateway 只选择已启用且已收敛的 HTTP 路由，再通过 VPS 回环端口和 SSH 隧道到达 Agent 的本地目标。
